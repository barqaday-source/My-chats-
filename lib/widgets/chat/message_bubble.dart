...
class _MessageBubbleState extends State<MessageBubble> {
  ...
  Future<void> _deleteMessage() async {
    try {
      ...
      // FIXED: fallback لأسماء الأعمدة القديمة/الجديدة
      final imageUrl = widget.message['media_url']?? widget.message['image_url'];
      if (imageUrl!= null) {
        final imagePath = _getFileName(imageUrl);
        try { await supabase.storage.from('chat_images').remove([imagePath]); } catch(_){}
      }

      final audioUrl = widget.message['audio_url']?? widget.message['voice_url'];
      if (audioUrl!= null) {
        final voicePath = _getFileName(audioUrl);
        try { await supabase.storage.from('voice_messages').remove([voicePath]); } catch(_){}
        try { await supabase.storage.from('chat_images').remove([voicePath]); } catch(_){} // FIXED: كان الصوت ينرفع بالغلط لـ chat_images
      }
  ...
  Widget build(BuildContext context) {
    // FIXED: fallback للأعمدة القديمة
    final imageUrl = widget.message['media_url']?? widget.message['image_url'];
    final audioUrl = widget.message['audio_url']?? widget.message['voice_url'];
